...
describe('MovieDetailsForm', () => {
    ...
    it('should validate TITLE', async () => {
        const { getByRole, getByText } = render(<MovieDetailsForm  movieRecord={EMPTY_MOVIE} onFormSubmit={onFormSubmit} isEdit />);
        const nameTextField = getByRole('textbox', { name: 'TITLE'});
...
test('should call onFormSubmit with changed record when submit button is clicked', async () => {
    const { getByRole, getByPlaceholderText } = render(<MovieDetailsForm  movieRecord={EMPTY_MOVIE} onFormSubmit={onFormSubmit} isEdit={false} />);
    const submitButton = getByRole('button', { name: 'SUBMIT' });
    const nameTextField = getByRole('textbox', { name: 'TITLE'});
    fireEvent.change(nameTextField, {
        target: {
